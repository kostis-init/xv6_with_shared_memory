...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
  if(size < 0 || (uint)i >= KERNBASE || (uint)i + size >= KERNBASE || ((uint)i >= curproc->sz && (uint)i < SHMEMEND) ||((uint)i+size > curproc->sz &&(uint)i < SHMEMEND))
    return -1;
  *pp = (char*)i;
  return 0;
}
...
extern int sys_nprocs_key(void);
